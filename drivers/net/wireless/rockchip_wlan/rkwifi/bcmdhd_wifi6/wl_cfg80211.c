...
extern void HdfInformBssFrameEventCallback(struct net_device *ndev, const struct InnerBssInfo *innerBssInfo);
extern int32_t HdfConnectResultEventCallback(struct net_device *ndev, const struct InnerConnetResult *innerConnResult);
...
wl_release_vif_macaddr(struct bcm_cfg80211 *cfg, u8 *mac_addr, u16 wl_iftype)
{
	...
	if ((wl_iftype == WL_IF_TYPE_P2P_DISC) || (wl_iftype == WL_IF_TYPE_AP) ){//||
		 ...
		 * fixed mac addr.
		 */
		WL_ERR(("%s: release Mac addr" MACDBG "\n",
			__FUNCTION__, MAC2STRDBG(mac_addr)));
		return BCME_OK;
	}
	...
	struct wireless_dev *wdev, char *ifname)
{
	...
	if (!wdev->netdev) {
		WL_ERR(("dead mac ndev null! \n"));
	} else {
		/* Disable tx before del */
		memcpy(mac_addr, wdev->netdev->dev_addr, ETH_ALEN);
	}
	wl_iftype = netinfo->iftype;

#ifdef WL_CFG80211_P2P_DEV_IF
	if (wdev->iftype == NL80211_IFTYPE_P2P_DEVICE) {
		wl_release_vif_macaddr(cfg, mac_addr, wl_iftype);
...
static s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, wl_bss_info_t *bi, bool update_ssid)
{
	...
	chanspec_t chanspec;
#ifdef CONFIG_AP6XXX_WIFI6_HDF
	struct InnerBssInfo innerBssInfo;
	struct net_device *ndev = bcmcfg_to_prmry_ndev(cfg);
#endif
	...
	WL_SCAN(("###0331###BSSID %pM, channel %3d(%3d %3sMHz), rssi %3d, capa 0x%-4x, mgmt_type %d, "
		"frame_len %3d, SSID \"%s\"\n",
...
#ifdef CONFIG_AP6XXX_WIFI6_HDF
	innerBssInfo.channel = channel;
	innerBssInfo.signal = signal;
	innerBssInfo.freq = freq;
	innerBssInfo.mgmt = mgmt;
	innerBssInfo.mgmtLen = notif_bss_info->frame_len;
	HdfInformBssFrameEventCallback(ndev, &innerBssInfo);
#endif
	...
	const wl_event_msg_t *e, void *data, bool completed, struct cfg80211_bss *bss)
{
	...
	struct InnerConnetResult innerConnResult;
	...
	innerConnResult.bssid = curbssid;
	innerConnResult.reqIe = conn_info->req_ie;
	innerConnResult.rspIe = conn_info->resp_ie;
	innerConnResult.reqIeLen = conn_info->req_ie_len;
	innerConnResult.rspIeLen = conn_info->resp_ie_len;
	innerConnResult.freq = freq;
	innerConnResult.statusCode = connectStatus;
	innerConnResult.connectStatus = connectStatus;
	HdfConnectResultEventCallback(ndev, &innerConnResult);
