...
static void flush_scache_range(void *addr, unsigned long size)
{
	int inc;
	...
	inc   = cpu_last_level_cache_line_size();

	for (; start < end; start += inc)
		flush_cache_line(Cache_LEAF3, start);
}
			...
			int32_t hot_x, int32_t hot_y)
{
	...
	flush_scache_range(cma->vaddr, 32*32*4);
