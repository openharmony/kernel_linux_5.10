...
static int loongson_detect_chip(struct loongson_drm_device *ldev)
{
	...
	if (pdev) {
		ldev->chip = dc_7a1000;
		ldev->gpu_pdev = pci_get_device(PCI_VENDOR_ID_LOONGSON, 0x7a15, NULL);
	...
	if (pdev) {
		ldev->chip = dc_7a2000;
		ldev->gpu_pdev = pci_get_device(PCI_VENDOR_ID_LOONGSON, 0x7a25, NULL);
...
static int loongson_drm_load(struct drm_device *dev, unsigned long flags)
{
	...
	drm_fb_helper_remove_conflicting_framebuffers(NULL, "loongson-drmfb", false);
