...
/*
 * Clear all watch registers.
 */
void loongarch_clear_watch_registers(void)
{
	unsigned int i, j, ibst, dbst, dbcn;
...
{
	int i, j, dbcn;
	unsigned int cntl = 0x1e;
	struct loongarch_watch_reg_state *watches = &t->thread.watch;
	...
	for (i = 0; i < boot_cpu_data.watch_reg_use_cnt; i++) {
		...
		if (watches->irw[i] & (LA_WATCH_R | LA_WATCH_W)) {
			...
			watch_csrwr(0, LOONGARCH_CSR_DB0ASID + 8*i);
			watch_csrwr(cntl, LOONGARCH_CSR_DB0CTL + 8*i);
			...
			watch_csrwr(0, LOONGARCH_CSR_IB0ASID + 8*j);
			watch_csrwr(cntl, LOONGARCH_CSR_IB0CTL + 8*j);
...
{
	unsigned int i, j, ibst, dbst, dbc, dbcn;
	...
	for (i = 0; i < boot_cpu_data.watch_reg_use_cnt; i++) {
		...
		if (i < dbcn) {
			...
			if (dbst & (1u << i)) {
				dbc = watch_csrrd(LOONGARCH_CSR_DB0CTL + i*8);
