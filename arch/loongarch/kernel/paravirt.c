...
void __init kvm_spinlock_init(void)
{
	...
	if (!(pv_feature_disabled & (1 << KVM_FEATURE_PARAVIRT_SPINLOCK)) &&
		pv_feature_support(KVM_FEATURE_PARAVIRT_SPINLOCK)) {
...
static int __init set_pv_spinlock(char *str)
{
	pv_feature_disabled |= (1 << KVM_FEATURE_PARAVIRT_SPINLOCK);
	return 0;
}
early_param("no_pv_spinlock", set_pv_spinlock);
