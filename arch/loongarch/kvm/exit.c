...
static int _kvm_trap_handle_gspr(struct kvm_vcpu *vcpu)
{
	...
	case 0x0:
		...
		if (inst.reg2_format.opcode == 0x1B) {
			...
			if ((index == 2) || (vcpu->arch.gprs[rd] == 0))
				/*
 ...
 * cacop_op, idle_op iocsr ops the same */
...
/**
 * _kvm_handle_fpu_disabled() - Guest used lbt however it is disabled at host
 * @vcpu:       Virtual CPU context.
 *
 * Handle when the guest attempts to use lbt which hasn't been allowed
 * by the root context.
 */
static int _kvm_handle_lbt_disabled(struct kvm_vcpu *vcpu)
{
	struct kvm_run *run = vcpu->run;

	/*
	 * If guest LBT not present, the LBT operation should have been
	 * treated as a reserved instruction!
	 * If LBT already in use, we shouldn't get this at all.
	 */
...
static exit_handle_fn _kvm_fault_tables[EXCCODE_INT_START] = {
	...
	[EXCCODE_BTDIS]		= _kvm_handle_lbt_disabled,
};
