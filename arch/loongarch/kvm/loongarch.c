...
struct kvm_stats_debugfs_item vcpu_debugfs_entries[] = {
	...
	VCPU_STAT("tlbmiss_ld", excep_exits[EXCCODE_TLBL]),
	VCPU_STAT("tlbmiss_st", excep_exits[EXCCODE_TLBS]),
	VCPU_STAT("tlb_ifetch", excep_exits[EXCCODE_TLBI]),
	VCPU_STAT("tlbmod", excep_exits[EXCCODE_TLBM]),
	VCPU_STAT("tlbri", excep_exits[EXCCODE_TLBNR]),
	VCPU_STAT("tlbxi", excep_exits[EXCCODE_TLBNX]),
	VCPU_STAT("fp_disabled", excep_exits[EXCCODE_FPDIS]),
	VCPU_STAT("lsx_disabled", excep_exits[EXCCODE_LSXDIS]),
	VCPU_STAT("lasx_disabled", excep_exits[EXCCODE_LASXDIS]),
	VCPU_STAT("fpe", excep_exits[EXCCODE_FPE]),
	VCPU_STAT("watch", excep_exits[EXCCODE_WATCH]),
	VCPU_STAT("gspr", excep_exits[EXCCODE_GSPR]),
	VCPU_STAT("vz_gsfc", excep_exits[EXCCODE_GCM]),
	VCPU_STAT("vz_hc", excep_exits[EXCCODE_HVC]),
...
void kvm_arch_create_vcpu_debugfs(struct kvm_vcpu *vcpu, struct dentry *debugfs_dentry)
{
	...
	debugfs_create_file("pid", 0444, debugfs_dentry, vcpu, &vcpu_pid_fops);
	for (p = vcpu_debugfs_entries; p->name && p->kind == KVM_STAT_VCPU; ++p) {
		debugfs_create_file(p->name, 0444, debugfs_dentry,
...
int kvm_arch_check_processor_compat(void *rtn)
{
	return 0;
}
...
int kvm_arch_vcpu_ioctl_run(struct kvm_vcpu *vcpu)
{
	...
	if (vcpu->mmio_needed) {
		if (!vcpu->mmio_is_write)
			_kvm_complete_mmio_read(vcpu, vcpu->run);
		vcpu->mmio_needed = 0;
	} else if (vcpu->arch.is_hypcall) {
		/* set return value for hypercall v0 register */
		vcpu->arch.gprs[REG_A0] = vcpu->run->hypercall.ret;
		vcpu->arch.is_hypcall = 0;
	}

	if (vcpu->run->exit_reason == KVM_EXIT_LOONGARCH_IOCSR) {
		if (!vcpu->run->iocsr_io.is_write)
			_kvm_complete_iocsr_read(vcpu, vcpu->run);
	}
	...
	_kvm_check_requests(vcpu, cpu);
	_kvm_update_vmid(vcpu, cpu);
	r = kvm_enter_guest(vcpu->run, vcpu);
		...
		const struct kvm_one_reg *reg, s64 *v)
{
	...
	switch (reg->id) {
	case KVM_REG_LBT_SCR0:
		*v = vcpu->arch.lbt.scr0;
		break;
	case KVM_REG_LBT_SCR1:
		*v = vcpu->arch.lbt.scr1;
		break;
	case KVM_REG_LBT_SCR2:
		*v = vcpu->arch.lbt.scr2;
		break;
	case KVM_REG_LBT_SCR3:
		*v = vcpu->arch.lbt.scr3;
		break;
	case KVM_REG_LBT_FLAGS:
		*v = vcpu->arch.lbt.eflags;
		break;
	case KVM_REG_LBT_FTOP:
		...
		s64 v)
{
	...
	case KVM_REG_LOONGARCH_COUNTER:
		 ...
		 */
		if (!vcpu->kvm->arch.stablecounter_gftoffset)
	...
	case KVM_REG_LOONGARCH_VCPU_RESET:
		...
		vcpu->kvm->arch.stablecounter_gftoffset = 0;
		break;
	default:
		if ((reg->id & KVM_REG_LOONGARCH_MASK) != KVM_REG_LOONGARCH_CSR)
			return -EINVAL;
...
void kvm_own_lbt(struct kvm_vcpu *vcpu)
{
	preempt_disable();
	if (!(vcpu->arch.aux_inuse & KVM_LARCH_LBT)) {
		kvm_set_csr_euen(KVM_EUEN_LBTEN);

		/* If guest lbt state not active, restore it now */
		kvm_restore_lbt(vcpu);
...
static void kvm_enable_lbt_fpu(struct kvm_vcpu *vcpu, unsigned long fcsr)
{
	 ...
	 * cause lbt exception, here enable lbt in advanced
	 */
	if (fcsr & FPU_CSR_TM)
		kvm_own_lbt(vcpu);
}
...
static void kvm_lose_lbt(struct kvm_vcpu *vcpu)
{
	...
	if (vcpu->arch.aux_inuse & KVM_LARCH_LBT) {
		kvm_save_lbt(vcpu);
		kvm_clear_csr_euen(KVM_EUEN_LBTEN);
		vcpu->arch.aux_inuse &= ~KVM_LARCH_LBT;
	}
	preempt_enable();
}
...
void kvm_own_lsx(struct kvm_vcpu *vcpu)
{
	...
	kvm_enable_lbt_fpu(vcpu, vcpu->arch.fpu.fcsr);
	kvm_set_csr_euen(KVM_EUEN_LSXEN | KVM_EUEN_FPEN);
...
void kvm_own_lasx(struct kvm_vcpu *vcpu)
{
	...
	kvm_enable_lbt_fpu(vcpu, vcpu->arch.fpu.fcsr);
	kvm_set_csr_euen(KVM_EUEN_FPEN | KVM_EUEN_LSXEN | KVM_EUEN_LASXEN);
...
void kvm_lose_fpu(struct kvm_vcpu *vcpu)
{
	...
	preempt_disable();
#ifdef CONFIG_CPU_HAS_LBT
	if (vcpu->arch.aux_inuse & KVM_LARCH_FP_ALL) {
		if (!(vcpu->arch.aux_inuse & KVM_LARCH_LBT)) {
	...
	if (vcpu->arch.aux_inuse & KVM_LARCH_LASX) {
		...
		kvm_clear_csr_euen(KVM_EUEN_FPEN | KVM_EUEN_LSXEN | KVM_EUEN_LASXEN);
	} else if (vcpu->arch.aux_inuse & KVM_LARCH_LSX) {
		kvm_save_lsx(vcpu);
		trace_kvm_aux(vcpu, KVM_TRACE_AUX_SAVE, KVM_TRACE_AUX_FPU_LSX);
		/* Disable LSX & FPU */
		kvm_clear_csr_euen(KVM_EUEN_FPEN | KVM_EUEN_LSXEN);
	...
	vcpu->arch.aux_inuse &= ~KVM_LARCH_FP_ALL;
