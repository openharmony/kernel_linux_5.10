			  ...
			  gpa_t addr, int len, const void *val)
{
	...
	if (8 == len) {
		...
		case LS7A_INT_CLEAR_OFFSET:
			/*
			 * For emulated device, only clear edge triggered irq
			 * on writing INTCLR reg no effect on level triggered
			 * irq
			 * However for pass-through device with level-triggered
			 * intx, here need clear interrupt
			 */
			old  = data;
			...
			data = old & ~state->intedge;
			ls7a_ioapic_unlock(kvm->arch.v_ioapic, &flags);
			for_each_set_bit(offset_tmp, &data, 64)
				kvm_notify_acked_irq(kvm, 0, offset_tmp);
			ls7a_ioapic_lock(kvm->arch.v_ioapic, &flags);
			break;
