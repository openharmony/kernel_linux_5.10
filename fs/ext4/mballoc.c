		       ...
		       struct ext4_buddy *e4b, gfp_t gfp)
{
	...
	if (page == NULL || !PageUptodate(page) || PageLocked(page)) {
		if (page)
			 ...
			 * migrations, since concurrent migrations can lead
			 * to bitmap inconsistency. And if we are not
			 * uptodate that implies somebody just created the
			 * page but is yet to initialize it. We can drop
			 * the page reference and try to get the page with
			 * lock in both cases to avoid concurrency.
			 */
			put_page(page);
		page = find_or_create_page(inode->i_mapping, pnum, gfp);
		if (page) {
